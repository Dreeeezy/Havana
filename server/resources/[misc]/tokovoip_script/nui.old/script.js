...
function checkPluginVersion(){
	if (isPluginVersionCorrect()){
		document.getElementById('pluginVersion').innerHTML = `VERSÃO: <font color="green">${voip.pluginVersion}</font>`;
	} else {
		document.getElementById('pluginVersion').innerHTML = `VERSÃO: <font color="red">${voip.pluginVersion}</font> (Required: ${voip.minVersion})`;
...
function updateTokovoipInfo(msg){
	...
	switch (voipStatus) {
		...
		case PLUGIN_INITIALIZING:
			...
			break;
		case WRONG_SERVER:
			msg = `Connected to the wrong TeamSpeak server, please join the server.</font>`;
			screenMessage = 'Servidor errado';
			color = 'red';
			break;
		case WRONG_CHANNEL:
			...
			screenMessage = 'Canal errado';
			color = 'red';
			break;
		case INCORRECT_VERSION:
			msg = 'Using incorrect plugin version';
			screenMessage = 'Versão incorreta';
			color = 'red';
			break;
		case OK:
			color = '#01b0f0';
	...
	document.getElementById('pluginStatus').innerHTML = `STATUS: <font color="${color}">${screenMessage || msg}</font>`;
}

function updateConfig(payload){
	voip = payload;
	document.getElementById('TSChannel').innerHTML = `CANAL: <font color="#01b0f0">${(voip.plugin_data.TSChannelWait) ? voip.plugin_data.TSChannelWait : voip.plugin_data.TSChannel}</font>`;
}
