if not IsDuplicityVersion() then -- CLIENT
    Utils = {
        ...
        ShowHelpNotification = function(msg, thisFrame, beep, duration)
            AddTextEntry('slotsNotification', msg)

            if thisFrame then
                DisplayHelpTextThisFrame('slotsNotification', false)
            else
                if beep == nil then
                    beep = true
                end
                BeginTextCommandDisplayHelp('slotsNotification')
    ...
    RegisterNetEvent('slots:showNotification')
    AddEventHandler(
