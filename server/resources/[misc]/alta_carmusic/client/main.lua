...
local nomidaberto
local SoundsPlaying = {}
Check = Tunnel.getInterface("alta_carmusic")
...
RegisterNUICallback("action", function(data)
	...
	elseif data.action == "play" then
		if xSound:soundExists(nameid) then
			if xSound:isPaused(nameid) then
				TriggerServerEvent("altamusic:ChangeState", true, nameid)
				local esperar = 0
				while nuiaberto do
					Wait(1000)
	...
	elseif data.action == "pause" then
		if xSound:soundExists(nameid) then
			if xSound:isPlaying(nameid) then
				TriggerServerEvent("altamusic:ChangeState", false, nameid)
			end
		end
	...
	elseif data.action == "loop" then
		if xSound:soundExists(nameid) then
			...
			TriggerServerEvent("altamusic:ChangeLoop",nameid,datasoundinfo.loop)
		else
	...
	elseif data.action == "forward" then
		if xSound:soundExists(nameid) then
			TriggerServerEvent("altamusic:ChangePosition", 10, nameid)
		end
...
function ApplySound(quanti,plate)
	...
	if vadi <= 1.01 and vadi >= -0.001 and exis then
		...
		TriggerServerEvent("altamusic:ChangeVolume", quanti, plate)
	end
end
...
function SetUrl(url,nid)
	...
	if url then
		...
		if encontrad then
			...
			TriggerServerEvent("altamusic:ModifyURL",vehdata)
		else
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				local veh = GetVehiclePedIsIn(PlayerPedId(),false)
				...
				vehdata.loop = datasoundinfo.loop
				TriggerServerEvent("altamusic:AddVehicle",vehdata)
			end
		end
	else
...
if Config.Permission then
	RegisterNetEvent("altamusic:ShowNui")
	AddEventHandler("altamusic:ShowNui", function()
		show()
	end)
end
...
function show(nomecenas)
    ...
    elseif nuiaberto then
		...
		TriggerEvent("Notify","importante","Você não pode fazer isso agora.",8000)
	end
end

Zones = {}

RegisterNetEvent("altamusic:AddVehicle")
AddEventHandler("altamusic:AddVehicle", function(data)
...
RegisterNetEvent("altamusic:ModifyURL")
...
RegisterNetEvent("altamusic:ChangeState")
AddEventHandler("altamusic:ChangeState", function(tipo, nome)
...
RegisterNetEvent("altamusic:ChangePosition")
...
RegisterNetEvent("altamusic:ChangeLoop")
AddEventHandler("altamusic:ChangeLoop", function(tipo, nome)
	if xSound:soundExists(nome) then
...
RegisterNetEvent("altamusic:ChangeVolume")
AddEventHandler("altamusic:ChangeVolume", function(som, range, nome)
	local carroe
	local crds
    for i = 1, #Zones do
        local v = Zones[i]
...
RegisterNetEvent("altamusic:SendData")
AddEventHandler("altamusic:SendData", function(data)
    Zones = data
    for i = 1, #Zones do
		local v = Zones[i]
		if v.isplaying then
