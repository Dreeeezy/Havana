$(document).ready(function() {
    ...
    btn.click(function() {
        $.post('http://alta_dj/togglePlaystate', JSON.stringify({}));
    ...
    directPlay.click(function() {
        $.post('http://alta_dj/playNewSong', JSON.stringify({
            link: $('#linkInput').val()
        }));
        return;
    });
    ...
    buttonTwo.click(function() {
        $.post('http://alta_dj/rewind', JSON.stringify({}));
        $('#timeLineInside').width($('#timeLineInside').width()-(perc*10))
        return;
    });

    buttonThree.click(function() {
        $.post('http://alta_dj/forward', JSON.stringify({}));
        $('#timeLineInside').width($('#timeLineInside').width()+(perc*10))
        return;
    });

    buttonFour.click(function() {
        $.post('http://alta_dj/down', JSON.stringify({}));
        return;
    });

    buttonFive.click(function() {
        $.post('http://alta_dj/up', JSON.stringify({}));
        return;
    });
    ...
    AddPlaylist.click(function() {
        ...
        if ($('#CreatePlaylistName').val() != '') {
            ...
            $.post('http://alta_dj/addPlayList', JSON.stringify({
    ...
    $(document).on('click', '.deleteSongs', function() {
        ...
        $.post('http://alta_dj/deleteSong', JSON.stringify({
    ...
    $(document).on('click', '.deletePlaylist', function() {
        ...
        $.post('http://alta_dj/deletePlaylist', JSON.stringify({
        ...
        if ($('#linkInput').val()) {
            ...
            $.getJSON('https://noembed.com/embed?url=', {format: 'json', url: $('#linkInput').val()}, function (data) {
                if (data.title) {
                    ...
                    $.post('http://alta_dj/addSongToPlaylist', JSON.stringify({
                        id: i.data('id'),
                        link: $('#linkInput').val()
                    }));
                    ...
                    $.post('http://alta_dj/noSongtitle', JSON.stringify({}));
                }
            });
    ...
    $(document).on('click', '.song', function() {
        ...
        $.post('http://alta_dj/playSongFromPlaylist', JSON.stringify({
            id: $(this).data('id'),
            link: $(this).data('link'),
    ...
    function Close(event) {
        if (event.keyCode === 27) {
            $('body').hide()
            $.post('http://alta_dj/close', JSON.stringify({}));
        }
    }
