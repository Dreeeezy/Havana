			    ...
			    if distance2 <= 1.1 and not andamento and hasRobberyStarted then
				    ...
				    if IsControlJustPressed(0,38) and not IsPedInAnyVehicle(ped) then
				    	if isAllPaintingsRobbed then
						    if GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_ASSAULTRIFLE") or GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_MICROSMG") or GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_MACHINEPISTOL") or GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_SPECIALCARBINE_MK2") or GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_ASSAULTSMG") then
