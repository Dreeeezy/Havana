    ...
    function(r, p)
        ...
        if b[source] == nil then
            ...
            if query[1].truck_upgrade >= y.ressuply[p.ressuply_id].truck_level then
                ...
                TriggerClientEvent("Notify", source, "negado", Lang[Config.lang]["upgrade_your_truck"])
            end
            SetTimeout(
                500,
                function()
...
AddEventHandler(
    ...
    function(r, z, C, D, type, x)
        ...
        if x then
            ...
            if Config.ESX.account_stores == "money" or Config.ESX.account_stores == "cash" then
                vRP.addBank(s,tonumber(x.reward) or 0)
            else
                vRP.addBank(s,tonumber(x.reward) or 0)
            end
        ...
        if b[source] == nil then
            ...
            if t then
                ...
                if parseInt(query.user_id) == t then
                    ...
                    MySQL.Sync.execute(o, {["@gas_station_id"] = r})
                    if Config.ESX.account_stores == "money" or Config.ESX.account_stores == "cash" then
                        vRP.addBank(t,Config.gas_station_locations[r].sell_price)
                    else
                        vRP.addBank(t,Config.gas_station_locations[r].sell_price)
                    end
