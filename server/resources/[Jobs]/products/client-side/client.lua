...
-- THREADSERVICE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("products:delivery")
AddEventHandler("products:delivery",function(source,args,rawCommand)
--RegisterCommand("delivery",function(source,args,rawCommand)
	...
	--if distance <= 5 then
		if inService then
			inService = false
			TriggerEvent("Notify","sucesso","Descartavel destruído.",3000)
			...
			timeselling()
			inService = true
			TriggerEvent("Notify","sucesso","Telefone descartavel ligado, espere um pouco que jaja alguem te chama.",5000)
		end
	--end
		...
		while true do
			if inService and inTimers > 0 then
				...
				if inTimers <= 0 then
					...
					TriggerEvent("NotifyPush",{ time = ("%H:%M:%S - %d/%m/%Y"), text = "Traz o de sempre!", code = "SMS", title = "Ainda trabalha com aquelas paradas?", x = pedLocate[rand][1], y = pedLocate[rand][2], z = pedLocate[rand][3], name = callName[math.random(#callName)].." "..callName2[math.random(#callName2)], rgba = {69,115,41} })
				end
			end
