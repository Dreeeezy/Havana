...
TriggerEvent("cancelando",true)
local ped = PlayerPedId()
	if not IsPedInAnyVehicle(ped) and not processo then
		TriggerEvent("Progress",5000,"Vasculhando...")
		processo = true
		vRP.playAnim(false,{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"},true)
		vRP.setStandBY(source,parseInt(Citizen.Wait(GetAnimDuration(animDict, "amb@prop_human_parking_meter@female@idle_a") * 550)))
		Wait(5000)
