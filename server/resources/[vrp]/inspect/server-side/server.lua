...
-- REVISTAR
-----------------------------------------------------------------------------------------------------------------------------------------
--[[ RegisterNetEvent("inspect:runInspect")
AddEventHandler("inspect:runInspect", function(source)
	local user_id = vRP.getUserId(source)
	if user_id then
		local nplayer = vRPclient.nearestPlayer(source,5)
		if nplayer then
			local nuser_id = vRP.getUserId(nplayer)
			if not vRP.hasPermission(nuser_id,"Policia") then
...
RegisterCommand("revistar",function(source,args,rawCommand)
	...
	if user_id then
		...
		if nplayer then
			local nuser_id = vRP.getUserId(nplayer)
			if not vRP.hasPermission(nuser_id,"Policia") or vRP.hasPermission(nuser_id,"Fib") then
				if vRP.hasPermission(user_id,"Policia") or vRP.hasPermission(nuser_id,"Fib") then
					vCLIENT.toggleCarry(nplayer,source)
					...
					if not vRP.wantedReturn(nuser_id) then
						local policia = vRP.numPermission("Policia") or vRP.numPermission("Fib")
						--if parseInt(#policia) > 2 then
							if vRPclient.getHealth(nplayer) > 101 then
								local request = vRP.request(nplayer,"Você está sendo revistado, você permite?",60)
								if request then
		...
		if user_id then
			if vRP.computeInvWeight(opened[user_id]) + vRP.itemWeightList(itemName) * parseInt(amount) <= vRP.getBackpack(opened[user_id]) then
				if vRP.itemSubTypeList(itemName) then
					if (vRP.getInventoryItemAmount(opened[user_id],itemName) + amount) > vRP.itemMaxAmount(itemName) then
						TriggerClientEvent("Notify",source,"negado","O jogador já possui esse tipo de item ou atingiu o limite.",5000)
					else
	...
	if itemName then
		...
		if user_id then
			if vRP.computeInvWeight(user_id) + vRP.itemWeightList(itemName) * parseInt(amount) <= vRP.getBackpack(user_id) then
				if vRP.itemSubTypeList(itemName) then
					if (vRP.getInventoryItemAmount(user_id,itemName) + amount) > vRP.itemMaxAmount(itemName) then
						TriggerClientEvent("Notify",source,"negado","Você já possui esse tipo de item ou atingiu o limite ou atingiu o limite.",5000)
					else
