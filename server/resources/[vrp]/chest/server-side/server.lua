	...
	if user_id then
		...
		if consult[1].name == chestName then
			if vRP.hasPermission(parseInt(user_id),consult[1].permiss) then
				...
				if inuse[chestName] == nil then
					inuse[chestName] = false
				end
				return true
			end
		end
...
function cnVRP.chestClose()
	...
	if user_id then
		if chestOpen[user_id] then
			local namechest = chestOpen[user_id]
			chestOpen[user_id] = nil
			inuse[namechest] = false
		end
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- CHECKOPEBN
-----------------------------------------------------------------------------------------------------------------------------------------
function cnVRP.checkOpen(chestname)
	if inuse[chestname] then
		return true
	else
		return false
	end
...
-- INUSECHEST
-----------------------------------------------------------------------------------------------------------------------------------------
function cnVRP.inuseChest(chestname, active)
	inuse[chestname] = active
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- OPENCHEST
-----------------------------------------------------------------------------------------------------------------------------------------
