...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookadmin = "https://discord.com/api/webhooks/896620622183034880/X7jQf6HynEfQRh854boPwTVB0mJMPcyR04sHwqGn0pzaqpQiwWJp3gcDmLepTGuWpKAJ"
local webhookkick = "https://discord.com/api/webhooks/896619378144083988/vGucQyLakGopsupyUgMoNViuceSsh05_Ng-IWv0-2q7gK0wEfthX-SNtLUf-79papq7X"
local webhookfac = "https://discord.com/api/webhooks/896619547749130271/-jd5__qmvaFsyEPn7ZGwc5bpnATSs0986tK9YcCYycPugBiM2DbaUjXDpuT8EJqiqI7-"
local webhookkeys = "https://discord.com/api/webhooks/896619651327479838/jF8kYFJNoKITpF_KOvz7KzfteNEhddNtYRQ9gyL0kseZ4dsNkfbvEbZ6mrxtDd6VyGmW"
local webhookcds = "https://discord.com/api/webhooks/896619760345841665/EMuiG0ErEIwPVf4UHU1eP5Ypqg0mPZW1eI07INRaFgWlqO0kvY3rPS9dmwhU2TMA46eT"
local webhookblacklist = "https://discord.com/api/webhooks/896619910904565760/1DGILMwrj1aK8CNebyG6lPRmnkNfIVEHbK4Tz8sS_EadYUmcTP188TYJKmDMWmvqTotG"
local webhookgive = "https://discord.com/api/webhooks/896620042882531348/h6OKzvTKVT7hiyaVO8Pfnv-zNd27sM8-PDPnlnJ3Vvx4UstFyTXlg0H5yfNOoKJKoulD"
local webhookban = "https://discord.com/api/webhooks/896620122024841226/LlhdbWItn_7SJU0m2qGINEl58YHK7Du4gjbaY3wsbhfNKdX0tRoDCXADGO9U3S0yUVaU"
...
local webhookunwl = "https://discord.com/api/webhooks/896620249003204608/WzXzGUpZyEboNPNBuSCE2UnG27hE49RIQqSh_luG2oatyZtslSrsOTOOBLJJ6YSyeSMO"
local webhookadmingod = "https://discord.com/api/webhooks/896620375813791785/DV_YFDpa1sfg97eWFmNpuUtHgG1TrzjfSPgmukSoA6Wz2JEO9qFAw8XQhazifiRILBbk"
local webhookaddcar = "https://discord.com/api/webhooks/896620494927831050/1RNtUslQ6bg3AEryQ3B4QSpMCtW8DspXW5wg-iNWUepFS3GMzJa5C9DloEDanTfm_8JY"
...
-- toogle
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("admmod",function(source,args,rawCommand)
    local user_id = vRP.getUserId(source)
    local identity = vRP.getUserIdentity(user_id)
    if user_id then
        if vRP.hasPermission(user_id,"Admin") then
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- PLATE COMANDO UTILIZADO PARA GERAR NOVAS PLACAS PARA VEICULOS SEM PLACA POR BUG
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("pl",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.hasPermission(user_id,"Admin") then
			local plate = vRP.generatePlateNumber(source)
			vRP.prompt(source,"Placa:",plate)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
	...
	if user_id then
		if vRP.hasPermission(user_id,"Admin") then
			if args[1] and args[2] and vRP.itemNameList(args[1]) ~= nil then
				...
				PerformHttpRequest(webhook.Item, function(err, text, headers) end, 'POST', json.encode({
					username = "Admin Item",
...
RegisterCommand("kick",function(source,args,rawCommand)
	...
	if user_id then
		if vRP.hasPermission(user_id,"Admin") and parseInt(args[1]) > 0 then
			...
			PerformHttpRequest(webhook.Kick, function(err, text, headers) end, 'POST', json.encode({
				username = "Admin Kick",
				avatar_url = webhook.Avatar,
				embeds = {
					{ 	------------------------------------------------------------
...
RegisterCommand("ban",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	local uidentity = vRP.getUserIdentity(user_id)
	local identity = vRP.getUserIdentity(parseInt(user_id))
	if user_id then
		if vRP.hasPermission(user_id,"Admin") and parseInt(args[1]) > 0 then
			...
			if identity then
				...
				PerformHttpRequest(webhook.Ban, function(err, text, headers) end, 'POST', json.encode({
					username = "Admin Ban",
					avatar_url = webhook.Avatar,
					embeds = {
						{ 	------------------------------------------------------------
...
RegisterCommand("wl",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.hasPermission(user_id,"Admin") or vRP.hasPermission(user_id,"Aprovador") then
			vRP.execute("vRP/set_whitelist",{ steam = tostring(args[1]), whitelist = 1 })
			TriggerClientEvent("Notify",source,"amarelo","Voce aprovou a WL de "..args[1]..".",3000)
	...
	if user_id then
		if vRP.hasPermission(user_id,"Owner") and parseInt(args[1]) > 0 and parseInt(args[2]) > 0 then
			...
			if identity then
				...
				TriggerClientEvent("Notify",source,"amarelo","Entregado <b>"..args[2].." Bitcoins</b> para ["..args[1].."]<b>"..identity.name.."</b>.",10000)
			end
		end
	end
end)
	...
	if user_id then
		if vRP.hasPermission(user_id,"Admin") and parseInt(args[1]) > 0 then
			...
			if identity then
				...
				PerformHttpRequest(webhook.Unban, function(err, text, headers) end, 'POST', json.encode({
					username = "Admin Unban",
					avatar_url = webhook.Avatar,
					embeds = {
						{ 	------------------------------------------------------------
							title = "Registro - /Unban",
							fields = {
								{ 
									name = "Usuário:", 
									value = ""..identity.name.." "..identity.name2.." [**"..user_id.."**] "
	...
	if user_id then
		if vRP.hasPermission(user_id,"Owner") then
			if not vRP.hasPermission(parseInt(args[1]),tostring(args[2])) then
				...
				PerformHttpRequest(webhook.Group, function(err, text, headers) end, 'POST', json.encode({
					username = "Admin Group",
					avatar_url = webhook.Avatar,
					embeds = {
						{ 	------------------------------------------------------------
							title = "Registro - Setagem",
							fields = {
								{ 
									name = "Staff:", 
									value = ""..identity.name.." "..identity.name2.." [**"..user_id.."**] "
								...
								{
									name = "Setou:", 
									value = "[**"..parseInt(args[1]).."**] "..identity2.name.." "..identity2.name2..""
								},
								{
									name = "Cargo:", 
									value = ""..args[2]..""
...
RegisterCommand("tuning",function(source,args,rawCommand)
	...
	if user_id then
		if vRP.hasPermission(user_id,"Owner") then
			...
			PerformHttpRequest(webhook.Tuning, function(err, text, headers) end, 'POST', json.encode({
				username = "Admin Tuning",
				avatar_url = webhook.Avatar,
				embeds = {
					{ 	------------------------------------------------------------
						title = "Registro - Tuning",
						fields = {
							{ 
								name = "Staff:", 
								value = ""..identity.name.." "..identity.name2.." [**"..user_id.."**] "
...
RegisterCommand("cone",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"Admin") or vRP.hasPermission(user_id,"Policia") or vRP.hasPermission(user_id,"Paramedico") then
	...
	if user_id then
		if vRP.hasPermission(user_id,"Admin") then
			...
			if vehicle then
				...
				PerformHttpRequest(webhook.Fix, function(err, text, headers) end, 'POST', json.encode({
					username = "Player Fix",
					avatar_url = webhook.Avatar,
					embeds = {
						{ 	------------------------------------------------------------
