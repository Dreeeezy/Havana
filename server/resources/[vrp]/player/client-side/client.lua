	...
	while true do
		...
		if IsPedArmed(ped,6) and GetGameTimer() >= (sprayTimers + 60000) and GetSelectedPedWeapon(ped) ~= GetHashKey("WEAPON_MUSKET") then
			...
			if IsPedShooting(ped) then
				...
				local coords = GetEntityCoords(ped)
				if (losSantos:isPointInside(coords) or sandyShores:isPointInside(coords) or paletoBay:isPointInside(coords)) or not policeService and not inPaintball then
					vSERVER.shotsFired()
				end
			end
		end
...
-----------------------------------------------------------------------------------------------------------------------------------------
local disService = {
	{ 310.11,-595.82,43.29,"Paramedico" }, -- Los Santos Cima
	{ 441.56,-982.5,30.69,"Policia" }, -- Los Santos
	{ 115.52,-747.91,45.76,"Fib" }, -- Fib
	{ 542.68,-200.24,54.5,"Mecanico" },
	{ -223.38,-1339.0,30.89,"Bennys" },
	{ 250.21,-1078.13,29.3,"Advogado" }
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- DISTANCESERVICE
-----------------------------------------------------------------------------------------------------------------------------------------
...
function cRP.repararDistance()
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	for k,v in pairs(repararCds) do
		local distance = #(coords - vector3(v[1],v[2],v[3]))
		if distance <= 10 then
			return true
		end
                ...
                if aim and IsPedInAnyVehicle(target) and not IsPedAPlayer(target) then
                    ...
                    if distance <= 10 and IsPedFacingPed(target,ped,180.0) and speed <= 5 then
                        ...
                        if not IsEntityDead(target) and distance <= 15 then
                            ...
                            TriggerEvent("Notify","importante","Você recebeu a Chave!",5000)
