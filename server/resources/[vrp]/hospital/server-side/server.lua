...
AddEventHandler("hospital:sangramento", function(source)
	local user_id = vRP.getUserId(source)
    if vRP.hasPermission(user_id,"Paramedico") then
        local nplayer = vRPclient.nearestPlayer(source,3)
        if nplayer then
...
AddEventHandler("hospital:diagnostico", function(source)
	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"Paramedico") then
		local nplayer = vRPclient.nearestPlayer(source,5)
		if nplayer then
...
AddEventHandler("hospital:tratamento", function(source)
	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"Paramedico") then
		local nplayer = vRPclient.nearestPlayer(source,5)
		if nplayer then
