...
-- config
local fov_max = 50.0
local fov_min = 10.0 -- nível máximo de zoom (fov menor = mais zoom)
local zoomspeed = 2.0 -- velocidade do zoom da câmera
local speed_lr = 3.0 -- velocidade que a câmera faz uma panorâmica da esquerda para a direita
local speed_ud = 3.0 -- velocidade que a câmera gira para cima e para baixo
...
--					Citizen.Trace("try to rappel")
					if GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed then
						...
						AddTextComponentString("~r~Você não pode fazer rapel deste assento")
...
----------------------------------------------------------------------------
local helicam = false
local polswift_hash = GetHashKey("swift")
local fov = (fov_max+fov_min)*0.5
local vision_state = 0 -- 0 is normal, 1 is nightmode, 2 is thermal vision
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
		if IsPlayerInPolswift() then
			local lPed = GetPlayerPed(-1)
			local heli = GetVehiclePedIsIn(lPed)
...
function IsPlayerInPolswift()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, polswift_hash)
end
