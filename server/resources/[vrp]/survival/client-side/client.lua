	...
	while true do
		...
		if not inPaintball and GetEntityHealth(ped) <= 101 and deathtimer >= 0 then
			if not deadPlayer then
				...
				if not IsEntityPlayingAnim(ped,"mini@cpr@char_b@cpr_str","cpr_kol_idle",3) and not IsPedInAnyVehicle(ped) then
				...
				if deathtimer > 0 then
					...
					drawTxt("~r~"..deathtimer.." Segundos para que os paramédicos cheguem.",4,0.5,0.93,0.50,255,255,255,255)
					...
					drawTxt("Digite /gg para desistir imediatamente.",4,0.5,0.93,0.50,255,255,255,255)
...
function cnVRP.finishDeath()
	local ped = PlayerPedId()
	local hospital = vector3(1120.79,-1557.02,35.39)
	local heading = 60.01
...
function cnVRP.startCure()
	...
	if cure then
		...
		until GetEntityHealth(ped) >= 170 or GetEntityHealth(ped) <= 101
