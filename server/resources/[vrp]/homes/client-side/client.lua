...
RegisterCommand("entrar",function(source,args)
	...
	for k,v in pairs(homesList) do
		...
		if distance <= 1.5 and vSERVER.checkPermissions(k) then
			...
			if v[1] == "Mansion01" then
				SetEntityCoords(ped,342.04,437.84,149.39)
				table.insert(internHouses,{ 342.04,437.84,149.39,"exit","SAIR" }) 
				table.insert(internHouses,{ 338.26,437.09,141.78,"vault","ABRIR" })
			end

			if v[1] == "Mansion02" then
				SetEntityCoords(ped,374.18,427.9,145.69)
				table.insert(internHouses,{ 374.18,427.9,145.69,"exit","SAIR" }) 
				table.insert(internHouses,{ 377.09,428.98,138.3,"vault","ABRIR" })
			end

			if v[1] == "Mansion03" then
				SetEntityCoords(ped,117.21,560.16,184.31)
				table.insert(internHouses,{ 117.21,560.16,184.31,"exit","SAIR" }) 
				table.insert(internHouses,{ 118.56,565.99,176.7,"vault","ABRIR" })
			end

			if v[1] == "Mansion04" then
				SetEntityCoords(ped,-174.37,497.73,137.66)
				table.insert(internHouses,{ -174.37,497.73,137.66,"exit","SAIR" }) 
				table.insert(internHouses,{ -174.58,493.88,130.05,"vault","ABRIR" })
			end

			if v[1] == "Mansion05" then
				SetEntityCoords(ped,-682.6,592.69,145.38)
				table.insert(internHouses,{ -682.6,592.69,145.38,"exit","SAIR" }) 
				table.insert(internHouses,{ -680.8,589.08,137.77,"vault","ABRIR" })
			end

			if v[1] == "Mansion06" then
				SetEntityCoords(ped,-571.94,662.22,145.84)
				table.insert(internHouses,{ -571.94,662.22,145.84,"exit","SAIR" }) 
				table.insert(internHouses,{ -568.46,667.07,138.24,"vault","ABRIR" })
			end

			if v[1] == "Apartament01" then
				SetEntityCoords(ped,-781.93,326.53,223.26)
				table.insert(internHouses,{ -781.93,326.53,223.26,"exit","SAIR" }) 
				table.insert(internHouses,{ -773.39,325.15,223.27,"vault","ABRIR" })
			end

			if v[1] == "Apartament02" then
				SetEntityCoords(ped,-785.21,323.61,212.0)
				table.insert(internHouses,{ -785.21,323.61,212.0,"exit","SAIR" }) 
				table.insert(internHouses,{ -766.73,328.46,211.4,"vault","ABRIR" })
			end

			if v[1] == "Apartament03" then
				SetEntityCoords(ped,-602.85,58.97,98.21)
				table.insert(internHouses,{ -602.85,58.97,98.21,"exit","SAIR" }) 
				table.insert(internHouses,{ -621.31,54.16,97.6,"vault","ABRIR" })
			end

			if v[1] == "Apartament04" then
				SetEntityCoords(ped,-1449.97,-525.9,69.56)
				table.insert(internHouses,{ -1449.97,-525.9,69.56,"exit","SAIR" }) 
				table.insert(internHouses,{ -1457.54,-529.88,69.57,"vault","ABRIR" })
			end

			if v[1] == "Apartament05" then
				SetEntityCoords(ped,-1452.13,-540.66,74.05)
				table.insert(internHouses,{ -1452.13,-540.66,74.05,"exit","SAIR" }) 
				table.insert(internHouses,{ -1466.59,-528.33,73.45,"vault","ABRIR" })
			end

			if v[1] == "Apartament06" then
				SetEntityCoords(ped,-890.75,-436.75,121.61)
				table.insert(internHouses,{ -890.75,-436.75,121.61,"exit","SAIR" }) 
				table.insert(internHouses,{ -898.47,-439.51,121.62,"vault","ABRIR" })
			end

			if v[1] == "Apartament07" then
				SetEntityCoords(ped,-912.65,-365.13,114.28)
				table.insert(internHouses,{ -912.65,-365.13,114.28,"exit","SAIR" }) 
				table.insert(internHouses,{ -926.71,-377.45,113.68,"vault","ABRIR" })
			end

			if v[1] == "Apartament08" then
				SetEntityCoords(ped,-907.06,-372.81,109.45)
				table.insert(internHouses,{ -907.06,-372.81,109.45,"exit","SAIR" }) 
				table.insert(internHouses,{ -914.8,-375.35,109.45,"vault","ABRIR" })
			end

			if v[1] == "Apartament09" then
				SetEntityCoords(ped,-269.65,-941.16,92.52)
				table.insert(internHouses,{ -269.65,-941.16,92.52,"exit","SAIR" }) 
				table.insert(internHouses,{ -273.71,-948.6,92.52,"vault","ABRIR" })
			end

			if v[1] == "Apartament10" then
				SetEntityCoords(ped,-31.47,-595.02,80.04)
				table.insert(internHouses,{ -31.47,-595.02,80.04,"exit","SAIR" }) 
				table.insert(internHouses,{ -12.63,-597.12,79.44,"vault","ABRIR" })
			end
			
			if v[1] == "Apartament11" then
				SetEntityCoords(ped,-18.6,-591.44,90.12)
				table.insert(internHouses,{ -18.6,-591.44,90.12,"exit","SAIR" }) 
				table.insert(internHouses,{ -25.76,-587.68,90.13,"vault","ABRIR" })
			end
					...
					if IsControlJustPressed(1,38) then
						...
						elseif v[4] == "vault" then
							if vSERVER.checkIntPermissions(tostring(houseOpen)) then
								Wait(1000)
								if not vSERVER.checkOpen(houseOpen) then
									SetNuiFocus(true,true)
									SendNUIMessage({ action = "showMenu" })
									TriggerEvent("sounds:source","chest",0.7)
									vSERVER.inuseChest(houseOpen, true)
								else
									TriggerEvent("Notify","vermelho","Baú em uso",5000)
								end
							end
						end
					end
