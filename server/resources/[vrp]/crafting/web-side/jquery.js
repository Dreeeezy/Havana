	...
	$.post("http://crafting/requestCrafting",JSON.stringify({ craft: selectCraft }),(data) => {
		...
		for (let x = 1; x <= data["maxweight"]; x++){
			...
			if (data.inventoryUser[slot] !== undefined) {
				...
				const item = `<div class="item populated" style="background-image: url('http://177.54.156.222/imagens/${v.index}.png'); background-position: center; background-repeat: no-repeat;" data-item-key="${v.key}" data-name-key="${v.name}" data-amount="${v.amount}" data-slot="${slot}">
					<div class="top">
		...
		for (let x = 1; x <= 50; x++){
			...
			if (nameList2[x - 1] !== undefined){
				...
				const item = `<div class="item populated" title="" style="background-image: url('http://177.54.156.222/imagens/${v.index}.png'); background-position: center; background-repeat: no-repeat;" data-item-key="${v.key}" data-name-key="${v.name}" data-list="${list}" data-slot="${slot}" data-desc="${v["desc"]}">
					<div class="top">
