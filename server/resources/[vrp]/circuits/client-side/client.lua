		...
		if inRunners then
			if raceTime > 0 then
				...
				if (raceTime <= 0 or not IsPedInAnyVehicle(PlayerPedId())) then
					TriggerServerEvent("circuits:explosivePlayers")
					raceTime = 0
					Points = 0
					cleanBlips()
					inRunners = false
					...
					TriggerEvent("Notify","vermelho","Você não conclui a corrida a tempo.",6000)
					Citizen.Wait(3000)
...
Citizen.CreateThread(function()
    while true do
        ...
        if inRunners then
            timeDistance = 1
            inTimers = string.format("%.2f",vSERVER.getClock() - initialTime)
        end
        if wantTimer ~= 0 then
            timeDistance = 1
            local timer = (vSERVER.getClock() - initialTime)
            local hours = (timer/3600)
            local minutes = (timer/60)
            local seconds = timer
            inTimers2 = string.format("%.0f:%.0f:%.0f", hours, minutes, seconds)
        end
